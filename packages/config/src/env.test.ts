...
describe('loadEnv()', () => {
  ...
  it('exits non-zero when DATABASE_URL is missing', () => {
    ...
    withEnv({ DATABASE_URL: undefined }, () => {
      ...
      expect(mockStderr).toHaveBeenCalledWith(expect.stringContaining('DATABASE_URL'));
  ...
  it('exits non-zero when REDIS_URL is missing', () => {
    ...
    withEnv({ REDIS_URL: undefined }, () => {
      expect(() => loadEnv()).toThrow('process.exit called');
      expect(mockStderr).toHaveBeenCalledWith(expect.stringContaining('REDIS_URL'));
    });
  ...
  it('rejects CORS_ALLOWED_ORIGINS = "*" in production', () => {
    ...
    withEnv({ NODE_ENV: 'production', CORS_ALLOWED_ORIGINS: '*' }, () => {
      expect(() => loadEnv()).toThrow('process.exit called');
      expect(mockStderr).toHaveBeenCalledWith(expect.stringContaining('CORS_ALLOWED_ORIGINS'));
    });
  ...
  it('rejects when DRIVER_PRICE_MAX_CENTS <= DRIVER_PRICE_MIN_CENTS', () => {
    ...
    withEnv({ DRIVER_PRICE_MIN_CENTS: '3000000', DRIVER_PRICE_MAX_CENTS: '500000' }, () => {
      expect(() => loadEnv()).toThrow('process.exit called');
      expect(mockStderr).toHaveBeenCalledWith(expect.stringContaining('DRIVER_PRICE_MAX_CENTS'));
    });
  ...
  it('rejects JWT_SECRET shorter than 32 characters', () => {
    ...
    withEnv({ JWT_SECRET: 'tooshort' }, () => {
      expect(() => loadEnv()).toThrow('process.exit called');
      expect(mockStderr).toHaveBeenCalledWith(expect.stringContaining('JWT_SECRET'));
    });
